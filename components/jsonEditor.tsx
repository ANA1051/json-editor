...
import { ViewArrayRounded, ViewArraySharp } from '@material-ui/icons';
...
class JsonEditor extends React.Component<{classes:any, data: JSON}, JsonEditorState> {
	...
	private handleClick(input: string){
		try{
			var temp = JSON.parse(input);
		}
		catch(err){
			alert("Input error! Invalid JSON array!");
			return
		}
	...
	private handleFetch(){
		return axios.get('https://random-data-api.com/api/name/random_name?size=5')
    		.then(({data}) => {
      			// handle success
	...
	render() {
		return (
			<div>
				...
				<JsonInput handleClick={(input: string)=>this.handleClick(input)}/>
				<ControlPanel refetchData={()=>this.handleFetch()} />
			</div>
		)
	}
}
